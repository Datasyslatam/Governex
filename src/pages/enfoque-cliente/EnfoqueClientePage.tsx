...
const DOFA_STYLE: Record<DofaEncuestaItem['tipo'], { icon: string; clase: string }> = {
  ...
  Debilidad:   { icon: '', clase: 'debilidad' },
...
const EnfoqueClientePage: React.FC = () => {
  ...
  const renderEncuestaCard = (encuesta: EncuestaGenerada, variant: 'cliente' | 'proveedor') => (
    <div className={`enf-encuesta-card enf-encuesta-card--${variant}`}> <div className="enf-encuesta-card__header"> <span className="enf-encuesta-card__tag">{variant === 'cliente' ? ' Clientes' : ' Proveedores'}</span> <h3>{encuesta.titulo}</h3> <p>{encuesta.introduccion}</p> </div> <div className="enf-encuesta-card__body">
        {encuesta.categorias.map(cat => (
          <div key={cat.categoria} className="enf-categoria"> <h4>{cat.categoria}</h4> <ol>
              {cat.preguntas.map(p => (
                <li key={p.id}>
                  {p.texto}
                  {p.tipo === 'escala'
      ...
      </div> <div className="enf-encuesta-card__footer"> <button className="btn-secondary" onClick={() => copiarEncuesta(encuesta)}> Copiar texto</button> <button className="btn-primary" disabled={descargando === variant}
          onClick={() => descargarEncuestaPDF(encuesta, variant)}>
          {descargando === variant ? 'Generando PDF...' : '⬇ Descargar PDF interactivo'}
        </button> </div> </div>
  )
  ...
  return (
    <div className="enf-page">
      ...
      {/* HEADER */}
      <div className="enf-page__header"> <div className="enf-page__title-block"> <h1> Enfoque al Cliente</h1> <p>Determinación, comprensión y cumplimiento de los requisitos del cliente y partes interesadas</p> <span className="enf-page__clause">Cláusula 5.1.2</span> </div> </div>

      {/* TABS */}
      <div className="enf-tabs"> <button className={`enf-tab-btn ${activeTab === 'encuestas' ? 'active' : ''}`} onClick={() => setActiveTab('encuestas')}>
           Encuestas de Satisfacción
        </button> <button className={`enf-tab-btn ${activeTab === 'archivos' ? 'active' : ''}`} onClick={() => setActiveTab('archivos')}>
           Respuestas, PQRS y Análisis DOFA
        </button> </div>

      {/* ── TAB: ENCUESTAS ─────────────────────────────────────── */}
      {activeTab === 'encuestas' && (
        <div className="enf-tab-panel"> <div className="enf-ai-panel"> <div className="enf-ai-panel__info"> <span className="enf-ai-panel__icon"></span> <div> <strong>Generar encuestas con Governex IA</strong> <p>
            ...
            {errorIA && <div className="enf-ai-error"> {errorIA}</div>}
          </div>

          {!encuestas && !loadingIA && (
            <div className="enf-empty-state"> <div className="enf-empty-state__icon"></div> <h2>Aún no se han generado encuestas</h2> <p>Haz clic en <strong>"Generar Encuestas con IA"</strong> para crear automáticamente las encuestas de satisfacción de clientes y proveedores, basadas en el contexto de tu organización (§4.1).</p> </div>
          )}
      ...
      {activeTab === 'archivos' && (
        <div className="enf-tab-panel"> <div className="enf-notice">
            ℹ Los archivos de encuestas y registros de PQRS se almacenan de manera persistente en la base de datos de la organización. La detección del tipo de documento es automática al subirlos.
          </div>

          {/* SUBIDA DE ARCHIVOS */}
          <div className="panel enf-upload-panel"> <h3> Subir encuestas respondidas y PQRS</h3> <div className="enf-upload-row"> <label className="enf-upload-btn" title={!canEdit ? 'Tu rol no tiene permiso para esta acción' : undefined}>
                {procesandoArchivos ? 'Leyendo archivo(s)...' : ' Seleccionar archivo(s)'}
                <input type="file" multiple accept="application/pdf" onChange={handleFileUpload} disabled={procesandoArchivos || !canEdit} style={{ display: 'none' }} /> </label> </div> <p className="enf-upload-hint">
            ...
            {archivos.length === 0 ? (
              ...
              <table className="enf-table"> <thead><tr><th>Archivo</th><th>Tipo</th><th>Lectura</th><th>Tamaño</th><th>Fecha</th><th></th></tr></thead> <tbody>
                  {archivos.map(a => {
                    const resp = respuestasEncuestas.find(r => r.id === a.id)
                    return (
                      <tr key={a.id}> <td>{a.nombre}</td> <td><span className="enf-badge">{a.tipo}</span></td> <td>
                          {resp
                        ...
                        </td> <td>{a.sizeKB} KB</td> <td>{a.fecha}</td> <td style={{ display: 'flex', gap: '0.3rem' }}>
                          {a.url && (
                            <PermissionGuard recurso="enfoque_cliente" accion="leer"> <a href={a.url} download={a.nombre} target="_blank" rel="noopener noreferrer" className="btn-icon" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }} title="Descargar archivo">
                                ⬇
                              </a> </PermissionGuard>
                          )}
                          <PermissionGuard recurso="enfoque_cliente" accion="eliminar" mode="hide"> <button className="btn-icon danger" onClick={() => eliminarArchivo(a.id)}></button> </PermissionGuard> </td> </tr>
                    )
                  })}
                </tbody> </table>
            )}
          ...
          {/* PQRS */}
          <div className="panel enf-pqrs-panel"> <div className="enf-pqrs-panel__header"> <h3> Registro de PQRS</h3> <button className="btn-primary" onClick={abrirModalPqrs} disabled={!canEdit} title={!canEdit ? 'Tu rol no tiene permiso para esta acción' : undefined}>＋ Agregar PQRS</button> </div>

            {pqrsList.length === 0 ? (
              <div className="enf-empty-mini">No hay registros de PQRS en esta sesión.</div>
            ) : (
              <table className="enf-table"> <thead><tr><th>Tipo</th><th>Cliente / Proveedor</th><th>Fecha</th><th>Descripción</th><th>Estado</th><th></th></tr></thead> <tbody>
                  {pqrsList.map(p => (
                    <tr key={p.id}> <td><span className="enf-badge">{p.tipo}</span></td> <td>{p.origen}</td> <td>{p.fecha}</td> <td className="enf-table__desc">{p.descripcion}</td> <td> <select value={p.estado} onChange={e => cambiarEstadoPqrs(p.id, e.target.value as EstadoPqrs)} disabled={!canEdit}> <option value="Abierta">Abierta</option> <option value="En Proceso">En Proceso</option> <option value="Cerrada">Cerrada</option> </select> </td> <td style={{ display: 'flex', gap: '0.3rem' }}> <button className="btn-icon" title="Descargar PDF" onClick={() => descargarPqrsPDF(p)}>⬇</button> <PermissionGuard recurso="enfoque_cliente" accion="eliminar" mode="hide"> <button className="btn-icon danger" onClick={() => eliminarPqrs(p.id)}></button> </PermissionGuard> </td> </tr>
                  ))}
                </tbody> </table>
            )}
          </div>
          ...
          {/* ANÁLISIS DOFA */}
          <div className="panel enf-analisis-panel"> <div className="enf-analisis-panel__header"> <div> <h3> Análisis DOFA generado por IA</h3> <p className="enf-analisis-panel__subtitle">
                ...
                </p> </div> <button className="btn-ai-generate" onClick={analizarConIA} disabled={loadingAnalisis || !canEdit} title={!canEdit ? 'Tu rol no tiene permiso para esta acción' : undefined}>
                {loadingAnalisis ? 'Analizando...' : (analisis ? ' Reanalizar' : ' Generar Análisis DOFA')}
              </button> </div>

            {errorAnalisis && <div className="enf-ai-error"> {errorAnalisis}</div>}
            ...
            {analisis && (
              <> <p className="enf-analisis-resumen">{analisis.resumenEjecutivo}</p>
                {documentosAnalizados.length > 0 && (
                  <div className="enf-documentos-analizados"> <h4> Documentos y fuentes incluidos en este análisis:</h4> <ul>
                ...
                <div className="enf-dofa-grid">
                  {DOFA_ORDEN.map(tipo => {
                    ...
                    return (
                      <div key={tipo} className={`enf-dofa-cell enf-dofa-cell--${clase}`}> <h4>{icon} {tipo}{tipo !== 'Fortaleza' && tipo !== 'Debilidad' ? 's' : 'es'}</h4>
                        {items.length === 0
      ...
      {/* MODAL: Agregar PQRS */}
      {showPqrsModal && (
        <div className="modal-overlay" onClick={cerrarModalPqrs}> <div className="modal-card" style={{ maxWidth: '520px' }} onClick={e => e.stopPropagation()}> <div className="modal-header"> <h3> {pqrsGuardada ? 'PQRS guardada' : 'Agregar PQRS'}</h3> <button className="modal-close" onClick={cerrarModalPqrs}></button> </div>

            {!pqrsGuardada ? (
              <> <div className="modal-body"> <div className="form-group"> <label>Tipo</label> <select className="filter-input form-control" value={newPqrs.tipo}
                      onChange={e => setNewPqrs(p => ({ ...p, tipo: e.target.value as TipoPqrs }))} disabled={!canEdit}> <option value="Petición">Petición</option> <option value="Queja">Queja</option> <option value="Reclamo">Reclamo</option> <option value="Sugerencia">Sugerencia</option> </select> </div> <div className="form-group"> <label>Cliente / Proveedor</label> <input type="text" className="filter-input form-control" placeholder="Nombre"
                      value={newPqrs.origen} onChange={e => setNewPqrs(p => ({ ...p, origen: e.target.value }))} disabled={!canEdit} /> </div> <div className="form-group"> <label>Fecha</label> <input type="date" className="filter-input form-control"
                      value={newPqrs.fecha} onChange={e => setNewPqrs(p => ({ ...p, fecha: e.target.value }))} disabled={!canEdit} /> </div> <div className="form-group"> <label>Descripción</label> <textarea className="filter-input form-control" rows={4}
                      value={newPqrs.descripcion} onChange={e => setNewPqrs(p => ({ ...p, descripcion: e.target.value }))} disabled={!canEdit} /> </div> </div> <div className="modal-footer"> <button className="btn--secondary btn" onClick={cerrarModalPqrs}>Cancelar</button> <button className="btn-primary" onClick={agregarPqrs} disabled={!newPqrs.origen.trim() || !newPqrs.descripcion.trim() || !canEdit} title={!canEdit ? 'Tu rol no tiene permiso para esta acción' : undefined}>
                    Guardar
                  </button> </div> </>
            ) : (
              <div className="modal-body enf-pqrs-confirm"> <div className="enf-pqrs-confirm__icon"></div> <p>La PQRS de <strong>{pqrsGuardada.origen}</strong> quedó registrada en esta sesión.</p> <p className="enf-pqrs-confirm__hint">
                  Descárgala en PDF para subirla junto con las encuestas respondidas en la sección
                  "Encuestas Respondidas y PQRS".
                </p> <div className="modal-footer" style={{ marginTop: '1rem' }}> <button className="btn--secondary btn" onClick={cerrarModalPqrs}>Cerrar</button> <button className="btn-primary" onClick={() => descargarPqrsPDF(pqrsGuardada)}>⬇ Descargar PDF</button> </div> </div>
            )}
          </div> </div>
      )}
    </div>
