...
function categoriaIcon(cat: string): string {
  const map: Record<string, string> = {
    ...
    'Normativo / Legal': '', 'Estratégico': '', 'Otro': '',
  }
  return map[cat] ?? ''
}
...
const KPICard: React.FC<{
  ...
  <div className="pc-kpi" style={{ borderTop: `3px solid ${color}` }}> <span className="pc-kpi__icon">{icon}</span> <span className="pc-kpi__value" style={{ color }}>{value}</span> <span className="pc-kpi__label">{label}</span> </div>
)
...
const FormModal: React.FC<FormModalProps> = ({ initial, cambios, onSave, onClose }) => {
  ...
  return (
    <div className="iso-modal-overlay" onClick={onClose}> <div className="iso-modal pc-modal" onClick={e => e.stopPropagation()}> <div className="pc-modal__header"> <h2>{isEdit ? 'Editar Cambio' : ' Registrar Cambio Planificado'}</h2>
          {form.codigo && <span className="pc-modal__codigo">{form.codigo}</span>}
        </div>

        {/* Tabs */}
        <div className="iso-tabs"> <button className={`iso-tab-btn${tab === 'general'  ? ' active' : ''}`} onClick={() => setTab('general')}> General</button> <button className={`iso-tab-btn${tab === 'detalle'  ? ' active' : ''}`} onClick={() => setTab('detalle')}> Detalle</button> <button className={`iso-tab-btn${tab === 'control'  ? ' active' : ''}`} onClick={() => setTab('control')}>Control</button> </div>

        {/* ── Tab General ─── */}
        {tab === 'general' && (
          <> <div className="iso-form-row"> <div className="iso-field"> <label>Categoría del cambio *</label> <select value={form.categoria} onChange={e => handleCategoria(e.target.value as CategoriaCambio)} disabled={isReadOnly()}>
                  {CATEGORIAS.map(c => <option key={c} value={c}>{categoriaIcon(c)} {c}</option>)}
                </select> </div> <div className="iso-field"> <label>Código</label> <input
                  value={form.codigo}
                  onChange={e => set('codigo', e.target.value)}
                  placeholder={generarCodigo(cambios, form.categoria)}
                  disabled={isEdit}
                /> </div> </div> <div className="iso-form-row iso-form-row--full"> <div className="iso-field"> <label>Descripción del cambio *</label> <textarea rows={3} value={form.descripcion} readOnly={isReadOnly()}
                  onChange={e => set('descripcion', e.target.value)}
                  placeholder="Describe el cambio planificado..." /> </div> </div> <div className="iso-form-row iso-form-row--full"> <div className="iso-field"> <label>Justificación / Propósito *</label> <textarea rows={2} value={form.justificacion} readOnly={isReadOnly()}
                  onChange={e => set('justificacion', e.target.value)}
                  placeholder="¿Por qué se realiza este cambio?" /> </div> </div> <div className="iso-form-row"> <div className="iso-field"> <label>Impacto *</label> <select value={form.impacto} onChange={e => set('impacto', e.target.value as ImpactoCambio)} disabled={isReadOnly()}> <option value="Alto">Alto</option> <option value="Medio">Medio</option> <option value="Bajo">Bajo</option> </select> </div> <div className="iso-field"> <label>Estado *</label> <select value={form.estado} onChange={e => set('estado', e.target.value as EstadoCambio)} disabled={isReadOnly()}>
        ...
        {/* ── Tab Detalle ─── */}
        {tab === 'detalle' && (
          <> <div className="iso-form-row iso-form-row--full"> <div className="iso-field"> <label>Acciones / Pasos para implementar el cambio</label> <textarea rows={3} value={form.acciones} readOnly={isReadOnly()}
                  onChange={e => set('acciones', e.target.value)}
                  placeholder={'1. Levantar requerimientos\n2. Asignar recursos\n3. Ejecutar\n4. Verificar'} /> </div> </div> <div className="iso-form-row iso-form-row--full"> <div className="iso-field"> <label>Implicaciones si no se realiza el cambio</label> <textarea rows={2} value={form.implicaciones} readOnly={isReadOnly()}
                  onChange={e => set('implicaciones', e.target.value)}
                  placeholder="Consecuencias de no ejecutar el cambio..." /> </div> </div> <div className="iso-form-row"> <div className="iso-field"> <label>Procesos afectados</label> <input value={form.procesos_afectados} onChange={e => set('procesos_afectados', e.target.value)} readOnly={isReadOnly()}
                  placeholder="Ej: Gestión de Calidad, Producción..." /> </div> <div className="iso-field"> <label>Documentos afectados</label> <input value={form.documentos_afectados} onChange={e => set('documentos_afectados', e.target.value)} readOnly={isReadOnly()}
                  placeholder="Ej: Manual de Calidad, Procedimientos..." /> </div> </div> <div className="iso-form-row"> <div className="iso-field"> <label>Recursos necesarios</label> <input value={form.recursos} onChange={e => set('recursos', e.target.value)} readOnly={isReadOnly()}
                  placeholder="Presupuesto, personal, equipos..." /> </div> </div> </>
        )}

        {/* ── Tab Control ─── */}
        {tab === 'control' && (
          <> <div className="iso-form-row"> <div className="iso-field"> <label>Responsable *</label> <input value={form.responsable} onChange={e => set('responsable', e.target.value)} readOnly={isReadOnly()}
                  placeholder="Director de Calidad, Gerente de TI..." /> </div> <div className="iso-field"> <label>Aprobado por</label> <input value={form.aprobado_por} onChange={e => set('aprobado_por', e.target.value)} readOnly={isReadOnly()}
                  placeholder="Alta dirección, Comité de Calidad..." /> </div> </div> <div className="iso-form-row"> <div className="iso-field"> <label>Fecha de inicio</label> <input type="date" value={form.fecha_inicio} onChange={e => set('fecha_inicio', e.target.value)} readOnly={isReadOnly()} /> </div> <div className="iso-field"> <label>Fecha de finalización</label> <input type="date" value={form.fecha_fin} onChange={e => set('fecha_fin', e.target.value)} readOnly={isReadOnly()} /> </div> </div> <div className="iso-form-row iso-form-row--full"> <div className="iso-field"> <label>Observaciones / Evidencias</label> <textarea rows={3} value={form.observaciones} readOnly={isReadOnly()}
                  onChange={e => set('observaciones', e.target.value)}
                  placeholder="Notas adicionales, resultados, lecciones aprendidas..." /> </div> </div> </>
        )}

        <div className="iso-modal__footer"> <button className="iso-btn-secondary" onClick={onClose}>Cancelar</button>
          {!isReadOnly() && (
...
const MatrizRow: React.FC<MatrizRowProps> = ({ cambio, onEdit, onDelete, onRetry }) => {
  ...
  return (
    <> <tr className="pc-matrix-row" onClick={() => setExpanded(v => !v)}> <td> <span className="pc-matrix__code">{cambio.codigo}</span>
          {cambio._sinGuardar && (
            <span className="pc-badge-singuardar" title="No se pudo guardar en la base de datos — usa 'Reintentar guardado'">
               Sin guardar
            </span>
          )}
        </td> <td> <span className="pc-categoria">
            {categoriaIcon(cambio.categoria)} {cambio.categoria}
          </span> </td> <td className="pc-matrix__desc"> <span className="pc-matrix__desc-text">{cambio.descripcion}</span> </td> <td className="pc-matrix__justif">{cambio.justificacion}</td> <td> <span className={`pc-impacto ${impactoClass(cambio.impacto)}`}>{cambio.impacto}</span> </td> <td style={{ fontSize: '0.8rem', whiteSpace: 'nowrap' }}>{cambio.responsable}</td> <td>{cambio.fecha_inicio && <span className="pc-fecha">{cambio.fecha_inicio}</span>}</td> <td>
          {cambio.fecha_fin && (
            <div className="pc-fecha-fin-cell"> <span className="pc-fecha">{cambio.fecha_fin}</span>
              {diasRestantes !== null && (
        ...
        </td> <td><span className={`pc-badge ${estadoClass(cambio.estado)}`}>{cambio.estado}</span></td> <td onClick={e => e.stopPropagation()}> <div className="pc-matrix__actions">
            {cambio._sinGuardar && (
            ...
            <button className="iso-btn-icon" title={!canEdit ? 'Tu rol no tiene permiso para editar' : "Editar"} onClick={() => onEdit(cambio)} disabled={!canEdit}></button> <PermissionGuard recurso="planificacion_cambios" accion="eliminar" mode="hide"> <button className="iso-btn-icon danger" title="Eliminar" onClick={() => onDelete(cambio.id)}></button> </PermissionGuard> </div> </td> </tr>

      {expanded && (
        <tr className="pc-expand-row"> <td colSpan={10}> <div className="pc-expand-content"> <div className="pc-expand-chips">
                {cambio.procesos_afectados && (
                  <div className="pc-expand-chip"> <span className="pc-expand-chip__label"> Procesos afectados</span> <span>{cambio.procesos_afectados}</span> </div>
                )}
                {cambio.documentos_afectados && (
                  <div className="pc-expand-chip"> <span className="pc-expand-chip__label"> Documentos afectados</span> <span>{cambio.documentos_afectados}</span> </div>
                )}
                {cambio.recursos && (
                  <div className="pc-expand-chip"> <span className="pc-expand-chip__label"> Recursos</span> <span>{cambio.recursos}</span> </div>
                )}
                {cambio.aprobado_por && (
                  <div className="pc-expand-chip"> <span className="pc-expand-chip__label">Aprobado por</span> <span>{cambio.aprobado_por}</span> </div>
                )}
              </div> <div className="pc-expand-cols">
                {cambio.acciones && (
                  <div className="pc-expand-block"> <h4> Acciones / Pasos</h4> <p>{cambio.acciones}</p> </div>
                )}
                {cambio.implicaciones && (
                  <div className="pc-expand-block"> <h4> Implicaciones si no se ejecuta</h4> <p>{cambio.implicaciones}</p> </div>
                )}
                {cambio.observaciones && (
                  <div className="pc-expand-block"> <h4> Observaciones</h4> <p>{cambio.observaciones}</p> </div>
                )}
              </div> </div> </td> </tr>
      )}
    </>
...
const PlanificacionCambiosPage: React.FC = () => {
  ...
  return (
    <div className="iso-page pc-page">
      ...
      {/* ── Header ──────────────────────────────────────── */}
      <header className="iso-page__header"> <div className="iso-page__title-block"> <nav className="pc-breadcrumb"> <span>Governex</span> <span className="pc-breadcrumb__sep">›</span> <span>Cap. 6.3</span> <span className="pc-breadcrumb__sep">›</span> <span className="pc-breadcrumb__active">Planificación de los Cambios</span> </nav> <h1>Planificación de los Cambios</h1> <p>Registro y seguimiento de todos los cambios planificados que afectan al SGC · Cláusula 6.3</p> <span className="iso-page__clause">Cláusula 6.3</span> </div> <button className="iso-btn-primary pc-btn-add" onClick={() => { setEditItem(null); setShowForm(true) }} disabled={!canCreate} title={!canCreate ? 'Tu rol no tiene permiso para esta acción' : undefined}>
           Registrar cambio
        </button> </header>

      {/* ── KPIs ────────────────────────────────────────── */}
      <div className="pc-kpis"> <KPICard value={total}       label="Total cambios"  color="#030097" icon="" /> <KPICard value={planif}      label="Planificados"   color="#7c3aed" icon="" /> <KPICard value={ejecucion}   label="En ejecución"   color="#d97706" icon="" /> <KPICard value={completados} label="Completados"    color="#059669" icon="" /> <KPICard value={altos}       label="Impacto alto"   color="#dc2626" icon="" /> <KPICard value={vencidos}    label="Vencidos"       color="#b91c1c" icon="⏰" /> </div>
      ...
      <div className="iso-topbar"> <span className="iso-topbar__info">
        ...
        </span> <div className="iso-topbar__actions"> <input className="pc-search" type="text" placeholder=" Buscar..."
            value={search} onChange={e => setSearch(e.target.value)} /> <select className="pc-filter" value={filterCat} onChange={e => setFilterCat(e.target.value)}> <option value="todos">Todas las categorías</option>
            {CATEGORIAS.map(c => <option key={c} value={c}>{categoriaIcon(c)} {c}</option>)}
          </select> <select className="pc-filter" value={filterImpacto} onChange={e => setFilterImpacto(e.target.value)}> <option value="todos">Todos los impactos</option> <option value="Alto"> Alto</option> <option value="Medio"> Medio</option> <option value="Bajo"> Bajo</option> </select> <select className="pc-filter" value={filterEstado} onChange={e => setFilterEstado(e.target.value)}> <option value="todos">Todos los estados</option>
            {ESTADOS.map(s => <option key={s} value={s}>{s}</option>)}
          </select> </div> </div>
      ...
      <div className="iso-table-wrapper pc-matrix-wrapper">
        {loading ? (
          ...
          <table className="iso-table pc-matrix"> <thead> <tr> <th>Código</th> <th>Categoría</th> <th>Descripción del cambio</th> <th>Justificación</th> <th>Impacto</th> <th>Responsable</th> <th>Inicio</th> <th>Fin</th> <th>Estado</th> <th>Acciones</th> </tr> </thead> <tbody>
              {filtered.map(cambio => (
                <MatrizRow
                  key={cambio.id}
                  cambio={cambio}
