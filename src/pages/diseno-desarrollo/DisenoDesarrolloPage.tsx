...
const datosIniciales: Proyecto[] = [
  { id: 1, entradas: 'Especificaciones funcionales, wireframes aprobados, API de ERP del cliente', desarrollo: 'App móvil de gestión de inventarios', control: 'APK/IPA firmadas, documentación técnica, manual de usuario', responsable: 'Jefe de Ingeniería', fechaInicio: '2025-01-15', fechaEntrega: '2025-05-30', etapa: 'Desarrollo', estado: 'En tiempo' },
  { id: 2, entradas: 'Normativa de etiquetado, especificaciones de material, restricciones de transporte', desarrollo: 'Rediseño de empaque industrial', control: 'Diseño aprobado, especificaciones de producción, prototipo validado', responsable: 'Director Técnico', fechaInicio: '2025-02-01', fechaEntrega: '2025-04-15', etapa: 'Verificación', estado: 'En riesgo' },
]

const empty = { entradas: '', desarrollo: '', control: '', responsable: '', fechaInicio: '', fechaEntrega: '', etapa: 'Planificación' as const, estado: 'En tiempo' as const }
  ...
  return (
    <div className="iso-page">
      ...
      <div className="iso-topbar">
        ...
        <button className="iso-btn-primary" onClick={() => setShowModal(true)}>＋ Diseño y Desarrollo</button>
      </div>

      <div className="iso-table-wrapper">
        <table className="iso-table">
          <thead>
            <tr><th>#</th><th>Entradas</th><th>Desarrollo</th><th>Control</th><th>Responsable</th><th>Inicio</th><th>Entrega</th><th>Etapa</th><th>Estado</th><th></th></tr>
          </thead>
          <tbody>
            {proyectos.map((p, i) => (
              <tr key={p.id}>
      ...
      {showModal && (
        <div className="iso-modal-overlay" onClick={() => setShowModal(false)}>
          <div className="iso-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="iso-field"><label>Desarrollo *</label><textarea rows={2} value={form.desarrollo} onChange={e => setForm(p => ({ ...p, desarrollo: e.target.value }))} /></div>
            <div className="iso-field"><label>Control</label><textarea rows={2} value={form.control} onChange={e => setForm(p => ({ ...p, control: e.target.value }))} /></div>
            ...
            <div className="iso-modal__footer">
              ...
              <button className="iso-btn-primary" onClick={guardar} disabled={!form.entradas || !form.desarrollo}>＋ Guardar</button>
            </div>
          </div>
        </div>
      )}
