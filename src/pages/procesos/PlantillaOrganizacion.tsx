...
import logoGovernex from "../../assets/logo_governex3.png";
...
async function generarPlantillaPDF(): Promise<Uint8Array> {
  ...
  page.drawRectangle({ x: 0, y: height - 72, width, height: 72, color: WHITE })
  page.drawLine({ start: { x: 0, y: height - 72 }, end: { x: width, y: height - 72 }, thickness: 2, color: NAVY })

  try {
    const logoPngBuffer = await imagenAPngArrayBuffer(logoGovernex)
    const logoImage     = await doc.embedPng(logoPngBuffer)
    const logoDims      = logoImage.scaleToFit(140, 55)
    page.drawImage(logoImage, {
      x: ML,
      y: height - 64 + (55 - logoDims.height) / 2,
      width:  logoDims.width,
      height: logoDims.height,
    })
  } catch {
    page.drawRectangle({ x: ML, y: height - 64, width: 140, height: 55,
      color: WHITE, borderColor: BLUE, borderWidth: 1.5 })
    page.drawText('GOVERNEX', { x: ML + 15, y: height - 40,
      font: fontBold, size: 16, color: BLUE })
  }

  page.drawText('PLANTILLA DE CONTEXTO ORGANIZACIONAL', {
    x: ML + 155, y: height - 30, font: fontBold, size: 11.5, color: NAVY,
  })
  page.drawText('Sistema de Gestión de Calidad', {
    x: ML + 155, y: height - 46, font: fontReg, size: 8.5, color: rgb(0.3, 0.4, 0.5),
  })
  page.drawText('Contexto de la Organización', {
    x: ML + 155, y: height - 59, font: fontReg, size: 7.5, color: rgb(0.4, 0.5, 0.6),
  })
  ...
  function drawSection(title: string, sub: string, yPos: number): number {
    page.drawRectangle({ x: ML, y: yPos - 30, width: CW, height: 30, color: NAVY })
    page.drawText(title, { x: ML + 10, y: yPos - 14, font: fontBold, size: 9.5, color: WHITE })
    page.drawText(sub, { x: ML + 10, y: yPos - 25, font: fontReg, size: 7, color: rgb(0.7, 0.85, 1) })
    return yPos - 30 - 10
  }
...
const PlantillaOrganizacion: React.FC<Props> = ({ currentDatos, onDatosYOrganigramaListos, onCancel }) => {
  ...
  const handleDescargar = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        html: `<div style="text-align:left;font-size:.875rem;line-height:1.7;color:#374151"> <p><strong>Pasos:</strong></p> <ol style="padding-left:1.25rem;margin:.4rem 0"> <li>Abre <strong>plantilla_governex_organizacion.pdf</strong></li> <li>Completa todos los campos marcados con <span style="color:#DC2626">*</span>.</li> <li>Guarda el archivo y cárgalo aquí con <strong>"Cargar Formulario"</strong>.</li> </ol> </div>`,
  ...
  return (
    <div className="plantilla-org panel">
      ...
      {/* Header */}
      <div className="plantilla-org__header"> <div className="plantilla-org__header-icon"></div> <div> <h3>Plantilla de Contexto Organizacional</h3> <p>Descarga el formulario PDF interactivo, complétalo, y cárgalo junto con el organigrama. Governex generará automáticamente la Misión, Visión y Política de Calidad.</p> </div> </div>
      ...
      <div className="plantilla-org__steps">
        {[
          ...
          <React.Fragment key={s.n}> <div className="plantilla-org__step"> <div className="plantilla-org__step-num">{s.n}</div> <div> <div className="plantilla-org__step-title">{s.t}</div> <div className="plantilla-org__step-desc">{s.d}</div> </div> </div>
      ...
      {currentDatos && (currentDatos.pdfFormularioUrl || currentDatos.organigramaUrl) && (
        <div className="plantilla-org__current-files"> <h4> Archivos del Análisis Actual</h4> <p className="plantilla-org__current-files-desc">Puedes descargar y revisar el formulario y organigrama que se encuentran activos actualmente en el SGC:</p> <div className="plantilla-org__current-grid">
            {currentDatos.pdfFormularioUrl && (
              <div className="plantilla-org__current-file-card"> <span className="current-file-icon"></span> <div className="current-file-info"> <div className="current-file-title">Formulario de Contexto</div> <div className="current-file-name">{currentDatos.pdfFormularioNombre || 'plantilla_contexto.pdf'}</div> </div> <button
                  type="button"
            ...
            {currentDatos.organigramaUrl && (
              <div className="plantilla-org__current-file-card"> <span className="current-file-icon"></span> <div className="current-file-info"> <div className="current-file-title">Organigrama de la Empresa</div> <div className="current-file-name">{currentDatos.organigramaNombre || 'organigrama.pdf'}</div> </div> <button
                  type="button"
                  className="btn-download-mini"
      ...
      {/* Botón descargar */}
      <button className="plantilla-org__btn-download" onClick={handleDescargar} disabled={generando}> <span className="plantilla-org__btn-icon">{generando ? '⏳' : '⬇'}</span> <div> <div className="plantilla-org__btn-title">
            {generando ? 'Generando formulario…' : 'Descargar Formulario PDF'}
          </div> <div className="plantilla-org__btn-sub">plantilla_governex_organizacion.pdf</div> </div> </button>

      {/* Dropzone: formulario */}
      <div className="plantilla-org__upload-label"> <span className="plantilla-org__upload-label-num">1</span>
      ...
      {parseado && (
        <div className="plantilla-org__ideario"> <div className="plantilla-org__ideario-header"> <div className="plantilla-org__ideario-title"> <span></span> <span>Misión, Visión y Política de Calidad</span> <span className="plantilla-org__ideario-badge">Generado por IA</span> </div> <button
          ...
          {generandoIdeario && !ideario ? (
            <div className="plantilla-org__ideario-loading"> <div className="plantilla-org__ideario-spinner" /> <span>Analizando datos de la empresa y redactando textos</span> </div>
          ) : idearioEditado ? (
            <div className="plantilla-org__ideario-fields">

              {/* Misión */}
              <div className="plantilla-org__ideario-field"> <label className="plantilla-org__ideario-label"> <span className="plantilla-org__ideario-label-icon"></span>
                  Misión
                  <span className="plantilla-org__ideario-editable-hint">Editable</span> </label> <textarea
              ...
              <div className="plantilla-org__ideario-field"> <label className="plantilla-org__ideario-label"> <span className="plantilla-org__ideario-label-icon"></span>
                ...
                /> </div> <p className="plantilla-org__ideario-hint">
      ...
      <div className="plantilla-org__upload-label"> <span className="plantilla-org__upload-label-num">2</span>
      ...
      > <input ref={orgFileRef} type="file"
        ...
        {cargandoOrg
          ...
          : archivoOrg && organigrama
            ? (<><span className="plantilla-org__drop-icon">{organigrama.mime === 'application/pdf' ? '' : ''}</span><span className="plantilla-org__drop-title">{archivoOrg.name}</span><span className="plantilla-org__drop-sub">Organigrama cargado correctamente</span></>)
            : (<><span className="plantilla-org__drop-icon"></span><span className="plantilla-org__drop-title">Cargar Organigrama</span><span className="plantilla-org__drop-sub">Arrastra o haz clic · PDF, JPG, JPEG, PNG o WEBP</span></>)}
      </div>

      {/* Footer */}
      <div className="plantilla-org__footer"> <button className="btn btn--secondary" onClick={onCancel}>Cancelar</button> <button
