...
const InfoChip: React.FC<{ icon: string; label: string; value: string }> = ({ icon, label, value }) =>
  value ? (
    <div className="ctx-chip"> <span className="ctx-chip__icon">{icon}</span> <div> <div className="ctx-chip__label">{label}</div> <div className="ctx-chip__value">{value}</div> </div> </div>
  ) : null
...
const TextBlock: React.FC<{
  ...
  <div className={`ctx-textblock ctx-textblock--${variant}`}> <div className="ctx-textblock__header"> <span className="ctx-textblock__icon">{icon}</span> <div className="ctx-textblock__titles"> <div className="ctx-textblock__title">{title}</div> <div className="ctx-textblock__subtitle">{subtitle}</div> </div>
      {badgeLabel && <span className="ctx-textblock__badge">{badgeLabel}</span>}
    </div> <p className="ctx-textblock__body">{text || <em className="ctx-empty">No definida aún.</em>}</p> </div>
)

const SectionHeader: React.FC<{ icon: string; title: string; clause?: string }> = ({
  icon, title, clause,
}) => (
  <div className="ctx-section-header"> <span className="ctx-section-header__icon">{icon}</span> <h4 className="ctx-section-header__title">{title}</h4>
...
const ExpandableText: React.FC<{ label: string; text: string }> = ({ label, text }) => {
  ...
  return (
    <div className="ctx-expandable"> <div className="ctx-expandable__label">{label}</div> <p className={`ctx-expandable__text ${!expanded && isLong ? 'ctx-expandable__text--clamped' : ''}`}>
...
const ContextoOrganizacionalPanel: React.FC<Props> = ({ datos, onEditar }) => {
  ...
  return (
    <div className="ctx-panel">
      ...
      <div className="ctx-header"> <div className="ctx-header__main"> <div className="ctx-header__name-row"> <h3 className="ctx-header__name">{datos.nombreEmpresa || 'Empresa sin nombre'}</h3>
            {datos.tipoEmpresa && <Badge label={datos.tipoEmpresa} variant="navy" />}
            {datos.sector && <Badge label={datos.sector} variant="blue" />}
          </div> <div className="ctx-header__chips"> <InfoChip icon="" label="Ubicación"      value={datos.ubicacion        ?? ''} /> <InfoChip icon="" label="Fundación"      value={datos.anoFundacion     ?? ''} /> <InfoChip icon="" label="Empleados"      value={datos.cantidadEmpleados ?? ''} /> <InfoChip icon="" label="Tamaño"         value={datos.tamano           ?? ''} /> <InfoChip icon="" label="Certificaciones" value={datos.certificaciones  ?? ''} /> </div> </div>
        {onEditar && (
          <button
      ...
      {tieneIdeario ? (
        <section className="ctx-section"> <SectionHeader
            icon=""
            title="Ideario Estratégico"
            clause="Cláusula 5.2"
          /> <div className="ctx-ideario-grid"> <TextBlock
              icon=""
              title="Misión"
        ...
        <div className="ctx-ideario-empty"> <span></span> <span>
      ...
      {tieneNarrativo && (
        <section className="ctx-section"> <SectionHeader icon="" title="Contexto Narrativo" clause="Cláusula 4.1" /> <div className="ctx-narrativo"> <ExpandableText label="" text={datos.contextoNarrativo ?? ''} /> </div> </section>
      )}

      {/* ── CONTEXTO OPERACIONAL ──────────────────────────────── */}
      <section className="ctx-section"> <SectionHeader icon="" title="Contexto Operacional" clause="Cláusulas 4.2, 4.3 y 8" /> <div className="ctx-operacional-grid"> <ExpandableText
