...
const Spinner: React.FC<{ text: string }> = ({ text }) => (
  <div style={{ position:'fixed', inset:0, background:'rgba(17,24,39,0.65)', zIndex:2000, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'1.25rem' }}> <div style={{ width:52, height:52, border:'5px solid rgba(255,255,255,0.2)', borderTopColor:'#fff', borderRadius:'50%', animation:'govSpin 0.75s linear infinite' }} /> <p style={{ color:'#fff', fontWeight:600, fontSize:'1rem', margin:0, textAlign:'center', maxWidth:340 }}>{text}</p> <p style={{ color:'rgba(255,255,255,0.6)', fontSize:'0.8rem', margin:0 }}>Analizando con Governex IA…</p> <style>{`@keyframes govSpin{to{transform:rotate(360deg)}}`}</style> </div>
)
...
const PanelGeneral: React.FC<{ ficha: FichaTecnica; onChange: (f: FichaTecnica) => void }> = ({ ficha, onChange }) => {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'0.85rem' }}> <div className="iso-form-row"> <div className="iso-field"><label>Cliente / Destinatario</label><input readOnly={isReadOnly()} value={ficha.cliente} onChange={e => set('cliente', e.target.value)} /></div> <div className="iso-field"><label>Producto / Servicio</label><input readOnly={isReadOnly()} value={ficha.productoServicio} onChange={e => set('productoServicio', e.target.value)} /></div> </div> <div className="iso-field"><label>Descripción general</label><textarea readOnly={isReadOnly()} rows={2} value={ficha.descripcion} onChange={e => set('descripcion', e.target.value)} /></div> <div className="iso-field"><label>Especificaciones técnicas</label><textarea readOnly={isReadOnly()} rows={3} value={ficha.especificacionesTecnicas} onChange={e => set('especificacionesTecnicas', e.target.value)} /></div> <div className="iso-form-row"> <div className="iso-field"><label>Normas / Estándares aplicables</label><input readOnly={isReadOnly()} value={ficha.normasAplicables} onChange={e => set('normasAplicables', e.target.value)} /></div> <div className="iso-field"><label>Condiciones de uso</label><input readOnly={isReadOnly()} value={ficha.condicionesUso} onChange={e => set('condicionesUso', e.target.value)} /></div> </div> <div className="iso-field"><label>Observaciones</label><textarea readOnly={isReadOnly()} rows={2} value={ficha.observaciones} onChange={e => set('observaciones', e.target.value)} /></div> <div className="iso-form-row"> <div className="iso-field"><label>Elaborado por</label><input readOnly={isReadOnly()} value={ficha.elaboradoPor} onChange={e => set('elaboradoPor', e.target.value)} /></div> <div className="iso-field"><label>Aprobado por</label><input readOnly={isReadOnly()} value={ficha.aprobadoPor} onChange={e => set('aprobadoPor', e.target.value)} /></div> </div> <div className="iso-form-row"> <div className="iso-field"><label>Versión</label><input readOnly={isReadOnly()} value={ficha.version} onChange={e => set('version', e.target.value)} /></div> <div className="iso-field"><label>Fecha elaboración</label><input type="date" readOnly={isReadOnly()} value={ficha.fechaElaboracion} onChange={e => set('fechaElaboracion', e.target.value)} /></div> </div> <div className="iso-field"><label>Estado</label> <select disabled={isReadOnly()} value={ficha.estado} onChange={e => set('estado', e.target.value as any)}> <option>En revisión</option><option>Vigente</option><option>Obsoleta</option> </select> </div> </div>
  )
}
...
const PanelEducativa: React.FC<{ ficha: FichaTecnica; onChange: (f: FichaTecnica) => void }> = ({ ficha, onChange }) => {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}> <div style={{ background:'#f8fafc', border:'1px solid #e5e7eb', borderRadius:'0.5rem', padding:'1rem', display:'flex', flexDirection:'column', gap:'0.75rem' }}> <p style={{ margin:0, fontWeight:700, fontSize:'0.82rem', color:'#030097' }}>Datos generales del área</p> <div className="iso-form-row"> <div className="iso-field"><label>Institución / Cliente</label><input readOnly={isReadOnly()} value={ficha.cliente} onChange={e => set('cliente', e.target.value)} /></div> <div className="iso-field"><label>Área / Asignatura</label><input readOnly={isReadOnly()} value={ficha.areaAsignatura} onChange={e => set('areaAsignatura', e.target.value)} /></div> </div> <div className="iso-field"><label>Objetivo general</label><textarea readOnly={isReadOnly()} rows={2} value={ficha.objetivoGeneral} onChange={e => set('objetivoGeneral', e.target.value)} /></div> <div className="iso-field"><label>Competencias a desarrollar</label><textarea readOnly={isReadOnly()} rows={2} value={ficha.competencias} onChange={e => set('competencias', e.target.value)} /></div> <div className="iso-field"><label>Observaciones</label><textarea readOnly={isReadOnly()} rows={2} value={ficha.observaciones} onChange={e => set('observaciones', e.target.value)} /></div> </div> <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}> <p style={{ margin:0, fontWeight:700, fontSize:'0.82rem', color:'#030097' }}>
          Cursos / Grados
          <span style={{ marginLeft:'0.5rem', background:'#dbeafe', color:'#1e40af', padding:'0.1rem 0.55rem', borderRadius:999, fontSize:'0.72rem', fontWeight:700 }}>{ficha.totalHorasSemana}h/semana</span> </p> <button disabled={!canEdit} title={!canEdit ? 'Tu rol no tiene permiso para esta acción' : undefined} className="iso-btn-primary" style={{ fontSize:'0.78rem', padding:'0.35rem 0.75rem' }} onClick={addU}>＋ Agregar curso</button> </div>
      {ficha.unidadesCurriculares.map((u, idx) => {
        const nc = NIVEL_COLOR[u.nivelCurso] || NIVEL_COLOR['Primaria']
        return (
          <div key={idx} style={{ border:`1.5px solid ${nc.color}35`, borderRadius:'0.6rem', padding:'1rem', background:nc.bg, display:'flex', flexDirection:'column', gap:'0.75rem' }}> <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}> <span style={{ fontWeight:700, fontSize:'0.82rem', color:nc.color }}> Curso #{idx+1}{u.gradoAnio && ` — ${u.gradoAnio}`}</span>
              {ficha.unidadesCurriculares.length > 1 && (
                <PermissionGuard recurso="requerimientos_ps" accion="eliminar" mode="hide"> <button className="iso-btn-icon danger" onClick={() => removeU(idx)}></button> </PermissionGuard>
              )}
            </div> <div className="iso-form-row"> <div className="iso-field"><label>Nivel educativo</label> <select disabled={isReadOnly()} value={u.nivelCurso} onChange={e => setU(idx,'nivelCurso',e.target.value as any)}> <option>Preescolar</option><option>Primaria</option><option>Secundaria</option><option>Media</option><option>Técnico</option> </select> </div> <div className="iso-field"><label>Grado / Año</label><input readOnly={isReadOnly()} value={u.gradoAnio} onChange={e => setU(idx,'gradoAnio',e.target.value)} placeholder="3° Primaria, 10° Grado…" /></div> </div> <div className="iso-form-row"> <div className="iso-field"><label>Nombre unidad / módulo</label><input readOnly={isReadOnly()} value={u.nombre} onChange={e => setU(idx,'nombre',e.target.value)} /></div> <div className="iso-field"><label>Intensidad horaria (h/sem)</label><input readOnly={isReadOnly()} type="number" min={1} max={40} value={u.intensidadHoraria} onChange={e => setU(idx,'intensidadHoraria',Number(e.target.value))} /></div> </div> <div className="iso-form-row"> <div className="iso-field"><label>Periodo académico</label><input readOnly={isReadOnly()} value={u.periodo} onChange={e => setU(idx,'periodo',e.target.value)} /></div> <div className="iso-field"><label>Docente responsable</label><input readOnly={isReadOnly()} value={u.docente} onChange={e => setU(idx,'docente',e.target.value)} /></div> </div> <div className="iso-field"><label>Contenido programático</label> <textarea readOnly={isReadOnly()} rows={4} value={u.contenidoProgramatico} onChange={e => setU(idx,'contenidoProgramatico',e.target.value)} placeholder="Tema 1: …&#10;Tema 2: …" /> </div> <div className="iso-form-row"> <div className="iso-field"><label>Metodología</label><input readOnly={isReadOnly()} value={u.metodologia} onChange={e => setU(idx,'metodologia',e.target.value)} /></div> <div className="iso-field"><label>Recursos y materiales</label><input readOnly={isReadOnly()} value={u.recursosMateriales} onChange={e => setU(idx,'recursosMateriales',e.target.value)} /></div> </div> <div className="iso-form-row"> <div className="iso-field"><label>Criterios de evaluación</label><input readOnly={isReadOnly()} value={u.criteriosEvaluacion} onChange={e => setU(idx,'criteriosEvaluacion',e.target.value)} /></div> <div className="iso-field"><label>Logros esperados</label><input readOnly={isReadOnly()} value={u.logros} onChange={e => setU(idx,'logros',e.target.value)} /></div> </div> </div>
        )
      })}
      <div style={{ background:'#f8fafc', border:'1px solid #e5e7eb', borderRadius:'0.5rem', padding:'1rem', display:'flex', flexDirection:'column', gap:'0.75rem' }}> <p style={{ margin:0, fontWeight:700, fontSize:'0.82rem', color:'#030097' }}> Control del documento</p> <div className="iso-form-row"> <div className="iso-field"><label>Elaborado por</label><input readOnly={isReadOnly()} value={ficha.elaboradoPor} onChange={e => set('elaboradoPor', e.target.value)} /></div> <div className="iso-field"><label>Aprobado por</label><input readOnly={isReadOnly()} value={ficha.aprobadoPor} onChange={e => set('aprobadoPor', e.target.value)} /></div> </div> <div className="iso-form-row"> <div className="iso-field"><label>Versión</label><input readOnly={isReadOnly()} value={ficha.version} onChange={e => set('version', e.target.value)} /></div> <div className="iso-field"><label>Fecha elaboración</label><input readOnly={isReadOnly()} type="date" value={ficha.fechaElaboracion} onChange={e => set('fechaElaboracion', e.target.value)} /></div> </div> <div className="iso-field"><label>Estado</label> <select disabled={isReadOnly()} value={ficha.estado} onChange={e => set('estado', e.target.value as any)}> <option>En revisión</option><option>Vigente</option><option>Obsoleta</option> </select> </div> </div> </div>
  )
}
...
const ModalFicha: React.FC<{ ficha: FichaTecnica; onChange: (f: FichaTecnica) => void; onClose: () => void; onGuardar: () => void; errorIA: string | null }> = ({ ficha, onChange, onClose, onGuardar, errorIA }) => {
  ...
  return (
  <div className="iso-modal-overlay" onClick={onClose}> <div className="iso-modal" style={{ maxWidth:720 }} onClick={e => e.stopPropagation()}> <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'0.5rem', flexWrap:'wrap' }}> <div> <h2 style={{ margin:0 }}>{ficha.tipo === 'educativa' ? '' : ''} Ficha Técnica</h2> <p style={{ margin:'0.15rem 0 0', fontSize:'0.78rem', color:'#6b7280' }}>
            {ficha.tipo === 'educativa' ? ficha.areaAsignatura : ficha.productoServicio} · {ficha.cliente}
          </p> </div> <div style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap', alignItems:'center' }}>
          {ficha.generadaConIA && <AIBadge />}
          <span className={`iso-badge ${ficha.estado === 'Vigente' ? 'verde' : ficha.estado === 'En revisión' ? 'amarillo' : 'gris'}`}>{ficha.estado}</span> </div> </div>
      ...
      <div className="iso-modal__footer"> <button className="iso-btn-secondary" onClick={onClose}>Cancelar</button> <button disabled={!canEdit} title={!canEdit ? 'Tu rol no tiene permiso para esta acción' : undefined} className="iso-btn-primary" onClick={onGuardar}>Guardar ficha</button> </div> </div> </div>
)}
...
const ModalVerFicha: React.FC<{ ficha: FichaTecnica; onClose: () => void; onEditar: () => void }> = ({ ficha, onClose, onEditar }) => {
  ...
  return (
    <div className="iso-modal-overlay" onClick={onClose}> <div className="iso-modal" style={{ maxWidth:700 }} onClick={e => e.stopPropagation()}> <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}> <div> <h2 style={{ margin:0, fontSize:'1.1rem' }}>{isEdu ? '' : ''} Ficha Técnica</h2> <p style={{ margin:'0.2rem 0 0', fontSize:'0.8rem', color:'#6b7280' }}>{ficha.id} · v{ficha.version} · {ficha.fechaElaboracion}</p> </div> <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'0.3rem' }}> <span className={`iso-badge ${ficha.estado === 'Vigente' ? 'verde' : ficha.estado === 'En revisión' ? 'amarillo' : 'gris'}`}>{ficha.estado}</span>
            {ficha.generadaConIA && <AIBadge />}
          </div> </div> <hr style={{ border:'none', borderTop:'1px solid #e5e7eb', margin:'0.25rem 0' }} /> <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem', fontSize:'0.82rem' }}> <div><span style={{ color:'#6b7280' }}>Cliente:</span> <strong>{ficha.cliente}</strong></div> <div><span style={{ color:'#6b7280' }}>{isEdu ? 'Área:' : 'Producto/Servicio:'}</span> <strong>{isEdu ? ficha.areaAsignatura : ficha.productoServicio}</strong></div> <div><span style={{ color:'#6b7280' }}>Elaborado por:</span> {ficha.elaboradoPor || '—'}</div> <div><span style={{ color:'#6b7280' }}>Aprobado por:</span> {ficha.aprobadoPor || '—'}</div> </div>
        {isEdu ? (
          <>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}> <strong style={{ fontSize:'0.85rem', color:'#030097' }}>Cursos ({ficha.unidadesCurriculares.length})</strong> <span style={{ fontSize:'0.78rem', background:'#dbeafe', color:'#1e40af', padding:'0.15rem 0.6rem', borderRadius:999, fontWeight:700 }}>Total: {ficha.totalHorasSemana}h/semana</span> </div>
            {ficha.unidadesCurriculares.map((u, i) => {
              const nc = NIVEL_COLOR[u.nivelCurso] || NIVEL_COLOR['Primaria']
              return (
                <div key={i} style={{ border:`1px solid ${nc.color}40`, borderRadius:'0.5rem', padding:'0.9rem', background:nc.bg, fontSize:'0.82rem', display:'flex', flexDirection:'column', gap:'0.4rem' }}> <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', alignItems:'center' }}> <span style={{ fontWeight:700, color:nc.color }}>{u.gradoAnio || `Curso #${i+1}`}</span> <span style={{ background:nc.bg, color:nc.color, border:`1px solid ${nc.color}60`, fontSize:'0.7rem', fontWeight:700, padding:'0.1rem 0.45rem', borderRadius:999 }}>{u.nivelCurso}</span> <span style={{ background:'#fff', border:'1px solid #e5e7eb', padding:'0.1rem 0.5rem', borderRadius:999, fontSize:'0.72rem', color:'#374151' }}>⏱ {u.intensidadHoraria}h/sem</span>
        ...
        <div className="iso-modal__footer"> <button className="iso-btn-secondary" onClick={onClose}>Cerrar</button> <button className="iso-btn-primary" onClick={onEditar}>Editar / Agregar</button> </div> </div> </div>
  )
}
...
const RequerimientosPSPage: React.FC = () => {
  ...
  return (
    <div className="iso-page">
      ...
      {/* Header */}
      <div className="iso-page__header"> <div className="iso-page__title-block"> <h1>Requerimientos para Productos y Servicios</h1> <p>Determinación, revisión y comunicación de requisitos — ISO 9001:2015 §8.2</p> <span className="iso-page__clause">Cláusula 8.2</span>
      ...
      {/* Info */}
      <div className="iso-info-box"> <span className="iso-info-box__icon"></span> <span> <strong>Cláusula 8.2</strong> — Governex IA genera automáticamente la matriz de revisiones a partir de los productos y servicios ingresados en el módulo 4.1.
      ...
      {/* Aviso sin contexto */}
      {sinContexto && (
        <div style={{ background:'#fffbeb', border:'1px solid #fde68a', borderRadius:'0.6rem', padding:'1rem 1.25rem', fontSize:'0.82rem', color:'#92400e', display:'flex', gap:'0.75rem', alignItems:'flex-start' }}> <span style={{ fontSize:'1.2rem', flexShrink:0 }}></span> <span> <strong>Sin análisis de IA disponible.</strong> Ve al módulo <strong>Contexto de la Organización (4.1)</strong>,
      ...
      {/* Error */}
      {errorMsg && (
        <div style={{ background:'#fee2e2', border:'1px solid #fca5a5', borderRadius:'0.6rem', padding:'0.75rem 1rem', fontSize:'0.82rem', color:'#991b1b', display:'flex', gap:'0.5rem', alignItems:'center' }}> <span></span> {errorMsg}
          <button style={{ marginLeft:'auto', background:'none', border:'none', color:'#991b1b', cursor:'pointer', fontSize:'1rem' }} onClick={() => setErrorMsg(null)}></button> </div>
      )}
      ...
      {tieneItems && (
        <div style={{ display:'flex', gap:'1rem', flexWrap:'wrap' }}>
          {[
            ...
            <div key={s.label} style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:'0.6rem', padding:'0.75rem 1.25rem', flex:1, minWidth:120 }}> <div>{s.icon}</div> <div style={{ fontSize:'1.4rem', fontWeight:700, color:s.color }}>{s.value}</div> <div style={{ fontSize:'0.75rem', color:'#6b7280' }}>{s.label}</div> </div>
          ))}
        </div>
      )}
      ...
      <div className="iso-topbar"> <div className="iso-topbar__info">
        ...
        </div> </div> <div style={{ display: 'flex', gap: '1rem', borderBottom: '2px solid #e5e7eb', marginBottom: '1.5rem', marginTop: '1rem' }}> <button
      ...
      {activeTab === 'matriz' && (
        <>
          {/* Estado vacío */}
          {!tieneItems ? (
        <div style={{ background:'#fff', border:'2px dashed #e5e7eb', borderRadius:'0.8rem', padding:'3.5rem 2rem', textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:'0.9rem' }}> <span style={{ fontSize:'3rem' }}>{esEducativo ? '' : ''}</span> <p style={{ margin:0, fontWeight:700, fontSize:'1.05rem', color:'#030097' }}>
            La matriz de revisiones se genera automáticamente con IA
          </p> <p style={{ margin:0, fontSize:'0.85rem', color:'#6b7280', maxWidth:460 }}>
            {sinContexto
        ...
        /* Tabla */
        <div className="iso-table-wrapper"> <table className="iso-table"> <thead> <tr> <th>#</th> <th>Cliente</th> <th>{esEducativo ? 'Área / Asignatura' : 'Producto / Servicio'}</th> <th>Requisitos del cliente</th> <th>Req. legales</th> <th>Req. organización</th> <th>Revisado por</th> <th>Fecha</th> <th>Ficha técnica</th> <th>Estado</th> <th></th> </tr> </thead> <tbody>
              {items.map((r, i) => {
                const fichaVinculada = r.fichaTecnicaId ? fichas[r.fichaTecnicaId] : null
                return (
                  <tr key={r.id}> <td style={{ color:'#9ca3af' }}> <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'0.15rem' }}> <span>{i+1}</span>
                        {r.generadoConIA && <AIBadge />}
                      </div> </td> <td style={{ fontWeight:600, color:'#030097' }}>{r.cliente}</td> <td>{r.productoServicio}</td> <td style={{ fontSize:'0.78rem', color:'#6b7280' }}>{r.requisitosCliente}</td> <td style={{ fontSize:'0.78rem', color:'#6b7280' }}>{r.requisitosLegales}</td> <td style={{ fontSize:'0.78rem', color:'#6b7280' }}>{r.requisitosOrg}</td> <td>{r.revisadoPor}</td> <td>{r.fechaRevision}</td> <td>
                      {fichaVinculada ? (
                        <button className="iso-btn-icon" style={{ fontSize:'0.75rem', padding:'0.4rem 0.8rem', color:'#6d28d9', borderColor:'#ddd6fe', background:'#f5f3ff', width: '100%' }} onClick={() => abrirVerFicha(r.fichaTecnicaId!)}>Ver Ficha Técnica</button>
                      ) : (
                        <button
                      ...
                      > <option value="Pendiente" style={{ color: '#000' }}>Pendiente</option> <option value="Aprobado" style={{ color: '#000' }}>Aprobado</option> <option value="Rechazado" style={{ color: '#000' }}>Rechazado</option> </select> </td> <td> <PermissionGuard recurso="requerimientos_ps" accion="eliminar" mode="hide"> <button className="iso-btn-icon danger" onClick={() => eliminarReq(r.id)}></button> </PermissionGuard> </td> </tr>
                )
              })}
            </tbody> </table> </div>
      )}
      ...
      {activeTab === 'fichas' && (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '0.8rem', padding: '1.5rem' }}> <h2 style={{ marginTop: 0, fontSize: '1.25rem', color: '#1e293b' }}>Fichas Técnicas Generadas</h2> <p style={{ color: '#6b7280', fontSize: '0.85rem', marginBottom: '1.5rem' }}>Aquí puedes consultar todas las Fichas Técnicas generadas para cada producto o servicio.</p> <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.25rem' }}>
            {items.map(req => {
              const hasFicha = !!req.fichaTecnicaId
              if (!hasFicha) return null
              
              return (
                <div key={req.id} style={{ border: '1px solid #e2e8f0', borderRadius: '0.5rem', padding: '1.25rem', background: '#f8fafc' }}> <div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '0.2rem' }}>{req.productoServicio}</div> <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '1rem' }}>Cliente: {req.cliente}</div> <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}> <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff', padding: '0.75rem', borderRadius: '0.4rem', border: '1px solid #cbd5e1' }}> <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#334155' }}>Ficha Técnica</span> <button className="iso-btn-icon" style={{ fontSize:'0.75rem', padding:'0.25rem 0.6rem', color:'#6d28d9', borderColor:'#ddd6fe', background:'#f5f3ff' }} onClick={() => abrirVerFicha(req.fichaTecnicaId!)}> Ver</button> </div> </div> </div>
              )
            })}
      ...
      {activeTab === 'control_ventas' && (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '0.8rem', padding: '1.5rem' }}> <h2 style={{ marginTop: 0, fontSize: '1.25rem', color: '#1e293b' }}> Control Comercial y Regulatorio</h2> <p style={{ color: '#6b7280', fontSize: '0.85rem', marginBottom: '1.5rem' }}>Aseguramiento de las variables de venta, cotizaciones, aprobación de planos/contratos y matrices legales para cada producto o servicio.</p> <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.25rem' }}>
            {items.map(req => {
              ...
              return (
                <div key={req.id} style={{ border: '1px solid #e2e8f0', borderRadius: '0.5rem', padding: '1.25rem', background: '#f8fafc' }}> <div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '0.2rem' }}>{req.productoServicio}</div> <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '1rem' }}>Cliente: {req.cliente}</div> <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}> <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff', padding: '0.75rem', borderRadius: '0.4rem', border: '1px solid #cbd5e1' }}> <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#334155' }}>
                        {hasControl ? 'Control diligenciado' : 'Pendiente de diligenciar'}
                      </span> <div style={{ display: 'flex', gap: '0.5rem' }}>
                        {hasControl && (
      ...
      {showReqModal && (
        <div className="iso-modal-overlay" onClick={() => setShowReqModal(false)}> <div className="iso-modal" onClick={e => e.stopPropagation()}> <h2> Agregar revisión manual</h2> <p style={{ margin:'-0.5rem 0 0.75rem', fontSize:'0.82rem', color:'#6b7280' }}>
              Completa los campos para agregar una revisión adicional a la matriz generada por IA.
            </p> <div className="iso-form-row"> <div className="iso-field"><label>Cliente *</label><input readOnly={isReadOnly()} value={form.cliente} onChange={e => setForm(p => ({ ...p, cliente:e.target.value }))} /></div> <div className="iso-field"><label>{esEducativo ? 'Área / Asignatura *' : 'Producto / Servicio *'}</label><input readOnly={isReadOnly()} value={form.productoServicio} onChange={e => setForm(p => ({ ...p, productoServicio:e.target.value }))} /></div> </div> <div className="iso-field"><label>Requisitos del cliente</label><textarea readOnly={isReadOnly()} rows={2} value={form.requisitosCliente} onChange={e => setForm(p => ({ ...p, requisitosCliente:e.target.value }))} /></div> <div className="iso-field"><label>Requisitos legales y reglamentarios</label><textarea readOnly={isReadOnly()} rows={2} value={form.requisitosLegales} onChange={e => setForm(p => ({ ...p, requisitosLegales:e.target.value }))} /></div> <div className="iso-field"><label>Requisitos de la organización</label><textarea readOnly={isReadOnly()} rows={2} value={form.requisitosOrg} onChange={e => setForm(p => ({ ...p, requisitosOrg:e.target.value }))} /></div> <div className="iso-form-row"> <div className="iso-field"><label>Revisado por</label><input readOnly={isReadOnly()} value={form.revisadoPor} onChange={e => setForm(p => ({ ...p, revisadoPor:e.target.value }))} /></div> <div className="iso-field"><label>Fecha</label><input type="date" readOnly={isReadOnly()} value={form.fechaRevision} onChange={e => setForm(p => ({ ...p, fechaRevision:e.target.value }))} /></div> </div> <div className="iso-field"><label>Estado</label> <select disabled={isReadOnly()} value={form.estado} onChange={e => setForm(p => ({ ...p, estado:e.target.value as any }))}> <option>Pendiente</option><option>Aprobado</option><option>Rechazado</option> </select> </div> <div className="iso-modal__footer"> <button className="iso-btn-secondary" onClick={() => setShowReqModal(false)}>Cancelar</button> <button className="iso-btn-primary" onClick={guardarReq} disabled={!form.cliente || !form.productoServicio || !canCreate} title={!canCreate ? 'Tu rol no tiene permiso para esta acción' : undefined}>＋ Agregar</button> </div> </div> </div>
      )}
      ...
      {/* Modal Control Comercial RF-018 */}
      {comercialModal && (
        <div className="iso-modal-overlay" onClick={() => { setComercialModal(null); setFileToUpload([]); setFileCotizacion([]); }}> <div className="iso-modal" style={{ maxWidth: 650 }} onClick={e => e.stopPropagation()}> <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}> <div> <h2 style={{ margin:0, color: '#1e3a8a', fontSize: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  Control Comercial y Regulatorio
                </h2> <p style={{ margin:'0.25rem 0 0', fontSize:'0.82rem', color:'#6b7280' }}>
                  Aseguramiento de variables de venta para: {comercialModal.productoServicio}
                </p> </div> </div> <div style={{ marginTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}> <div className="iso-field"> <label>Oferta Comercial & Requisitos</label> <textarea rows={3} value={comercialForm.cotizacion || ''} onChange={e => setComercialForm(p => ({ ...p, cotizacion: e.target.value }))} placeholder="Nº de cotización, especificaciones solicitadas, cantidades, precios..." readOnly={comercialMode === 'ver' || isReadOnly()} style={comercialMode === 'ver' ? { background: '#f8fafc', color: '#475569' } : {}} />
                ...
                {parseUrls(comercialForm.urlCotizacion).length > 0 && (
                   <div style={{display:'flex', flexDirection:'column', gap:'0.5rem', marginTop: '0.75rem'}}>
                     {parseUrls(comercialForm.urlCotizacion).map((url: string, i: number) => (
                       <div key={i} style={{ display:'flex', gap:'0.5rem', alignItems:'center' }}> <a href={getProxiedUrl(url)} target="_blank" rel="noreferrer" style={{color:'#2563eb', fontSize:'0.85rem', textDecoration:'none', fontWeight:600}}> Ver Cotización Adjunta {i + 1}</a>
                         {comercialMode !== 'ver' && (
                ...
                {comercialMode !== 'ver' && (
                  <div style={{ marginTop: '0.75rem' }}> <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#f0f9ff', border: '1px dashed #3b82f6', borderRadius: '0.5rem', cursor: 'pointer', textAlign: 'center' }}> <span style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}></span> <span style={{ fontWeight: 600, color: '#0369a1', marginBottom: '0.25rem' }}>
                      ...
                      </span> <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Arrastra o haz clic para subir cotizaciones • Solo .pdf, .docx, .xlsx</span> <input type="file" multiple accept=".pdf,.docx,.xlsx" onChange={e => setFileCotizacion(Array.from(e.target.files || []))} style={{ display: 'none' }} /> </label>
                    {fileCotizacion.length > 0 && (
                      <div style={{ display: 'flex', justifyContent: 'center', marginTop: '0.5rem' }}> <button className="iso-btn-secondary" onClick={async () => {
              ...
              </div> <div className="iso-field"> <label>Aprobación de la Capacidad Organizacional</label> <textarea rows={2} value={comercialForm.aprobacionInterna || ''} onChange={e => setComercialForm(p => ({ ...p, aprobacionInterna: e.target.value }))} placeholder="Detalla quién aprobó la oferta (ej. Gerencia Operativa confirma disponibilidad de personal y maquinaria)." readOnly={comercialMode === 'ver' || isReadOnly()} style={comercialMode === 'ver' ? { background: '#f8fafc', color: '#475569' } : {}} /> </div> <div className="iso-field"> <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}> <label style={{ margin: 0 }}>Cumplimiento Regulatorio y Matriz Legal</label>
                  {comercialMode !== 'ver' && (
                ...
                </div> <textarea rows={4} value={comercialForm.matrizLegal || ''} onChange={e => setComercialForm(p => ({ ...p, matrizLegal: e.target.value }))} placeholder="Legislación identificada, normas técnicas aplicables, permisos, registros regulatorios (INVIMA, ICA, RETIE, etc.)..." readOnly={comercialMode === 'ver' || isReadOnly()} style={comercialMode === 'ver' ? { background: '#f8fafc', color: '#475569' } : {}} /> </div> <div className="iso-field"> <label>Evidencia Documentada (Contrato, Orden de Compra, Planos)</label>
                {parseUrls(comercialForm.urlContrato).length > 0 && (
                   <div style={{display:'flex', flexDirection:'column', gap:'0.5rem', marginTop: '0.75rem'}}>
                     {parseUrls(comercialForm.urlContrato).map((url: string, i: number) => (
                       <div key={i} style={{ display:'flex', gap:'0.5rem', alignItems:'center' }}> <a href={getProxiedUrl(url)} target="_blank" rel="noreferrer" style={{color:'#2563eb', fontSize:'0.85rem', textDecoration:'none', fontWeight:600}}> Ver Documento de Evidencia {i + 1}</a>
                         {comercialMode !== 'ver' && (
                ...
                {comercialMode !== 'ver' && (
                  <div style={{ marginTop: '0.75rem' }}> <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: '#f0f9ff', border: '1px dashed #3b82f6', borderRadius: '0.5rem', cursor: 'pointer', textAlign: 'center' }}> <span style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}></span> <span style={{ fontWeight: 600, color: '#0369a1', marginBottom: '0.25rem' }}>
                      ...
                      </span> <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Arrastra o haz clic • Solo .pdf, .docx, .xlsx</span> <input type="file" multiple accept=".pdf,.docx,.xlsx" onChange={e => setFileToUpload(Array.from(e.target.files || []))} style={{ display: 'none' }} /> </label>
              ...
              </div> </div> <div className="iso-modal__footer" style={{ marginTop: '1.5rem' }}> <button className="iso-btn-secondary" onClick={() => { setComercialModal(null); setFileToUpload([]); setFileCotizacion([]); }}>{comercialMode === 'ver' ? 'Cerrar' : 'Cancelar'}</button>
              {comercialMode !== 'ver' && (
