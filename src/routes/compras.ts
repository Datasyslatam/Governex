...
router.post('/', requirePermission('compras', 'crear'), async (req: AuthRequest, res: Response) => {
  const { proveedor_id, nit_proveedor, proveedor, producto, cantidad, unidad,
  ...
  try {
    ...
    if (!finalProveedorId && proveedor) {
      const pRes = await pool.query('SELECT id FROM proveedores WHERE razon = $1 AND tenant_id = $2', [proveedor, tenantId])
      if (pRes.rows.length > 0) {
        finalProveedorId = pRes.rows[0].id
      } else {
        const dummyNit = nit_proveedor || ('PENDIENTE-' + Date.now())
        const iRes = await pool.query('INSERT INTO proveedores (nit, razon, tenant_id) VALUES ($1, $2, $3) RETURNING id', [dummyNit, proveedor, tenantId])
        finalProveedorId = iRes.rows[0].id
      }
    }
    if (finalProveedorId) {
      const { rowCount } = await pool.query(
        'SELECT 1 FROM proveedores WHERE id = $1 AND tenant_id = $2', [finalProveedorId, tenantId]
      )
      ...
      rows.map(async (ft) => {
        ...
        const signedDocs = await Promise.all(
          docs.map(async (d: any) => {
            if (typeof d === 'string') return { nombre: d, url: null }
            if (!d) return null
            return {
              nombre: d.nombre,
              url: d.url ? await resolveFileUrl(d.url, tenantId) : null,
            }
          })
        )
        return { ...ft, documentos_requeridos: signedDocs.filter(Boolean) }
      })
    )
...
router.post('/fichas-tecnicas', requirePermission('compras', 'crear'), async (req: AuthRequest, res: Response) => {
  const { nombre, descripcion, especificaciones, unidadMedida, cantidadMinima,
          documentosRequeridos, responsable, fechaCreacion, variablesCriticas, proveedor_id } = req.body
  if (!nombre) return res.status(400).json({ error: 'nombre es requerido' })
  try {
    const { rows } = await pool.query(
      `INSERT INTO fichas_tecnicas_compra
         (nombre, descripcion, especificaciones, unidad_medida, cantidad_minima,
          documentos_requeridos, responsable, fecha_creacion, creado_por, tenant_id, variables_criticas, proveedor_id)
      ...
      [
        ...
        variablesCriticas ? JSON.stringify(variablesCriticas) : null,
        proveedor_id || null
      ]
    )
    res.status(201).json(rows[0])
...
router.put('/fichas-tecnicas/:id', requirePermission('compras', 'editar'), async (req: AuthRequest, res: Response) => {
  const { id } = req.params
  const { nombre, descripcion, especificaciones, unidadMedida, cantidadMinima,
          documentosRequeridos, responsable, variablesCriticas, proveedor_id } = req.body
  try {
    const { rows } = await pool.query(
      `UPDATE fichas_tecnicas_compra
       SET nombre=$1, descripcion=$2, especificaciones=$3, unidad_medida=$4,
           ...
           variables_criticas=$10, proveedor_id=$11
      ...
      [nombre, descripcion || null, especificaciones || null, unidadMedida || null,
       ...
       variablesCriticas ? JSON.stringify(variablesCriticas) : null, proveedor_id || null]
    )
...
router.post('/evaluaciones', requirePermission('compras', 'crear'), async (req: AuthRequest, res: Response) => {
  const { ordenId, proveedor, producto, calidad, tiempoEntrega, diasRetraso,
          precio, capacidadRespuesta, puntajeGlobal, observaciones, variables_evaluadas } = req.body
    ...
    const { rows } = await pool.query(
      `INSERT INTO evaluaciones_orden_compra
         (orden_id, proveedor, producto, calidad, tiempo_entrega, dias_retraso,
          precio, capacidad_respuesta, puntaje_global, observaciones, creado_por, tenant_id, variables_evaluadas)
      ...
      [
        ...
        variables_evaluadas ? JSON.stringify(variables_evaluadas) : null
      ]
    )
    res.status(201).json(rows[0])
  } catch (err) {
...
router.put('/:id', requirePermission('compras', 'editar'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      [finalProveedorId || null, proveedor, producto, cantidad || null, unidad || null,
