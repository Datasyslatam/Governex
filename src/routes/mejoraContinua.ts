...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  try {
    const { rows } = await pool.query(
      `SELECT * FROM mejoras_continuas WHERE id=$1`, [req.params.id]
    )
    ...
    const { rows } = await pool.query(
      `INSERT INTO mejoras_continuas
         (codigo, titulo, origen, proceso, descripcion, beneficio_esperado,
          responsable, fecha_inicio, fecha_cierre, avance_pct, estado, proceso_id, creado_por)
  ...
  try {
    const { rows } = await pool.query(
      ...
      [titulo, origen, proceso || null, descripcion || null,
       ...
       avance_pct ?? 0, estado, proceso_id || null, id]
    )
    if (!rows[0]) return res.status(404).json({ error: 'Iniciativa no encontrada' })
    res.json(rows[0])
  } catch (err) {
...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
  try {
    const { rowCount } = await pool.query(
      `DELETE FROM mejoras_continuas WHERE id=$1`, [req.params.id]
    )
