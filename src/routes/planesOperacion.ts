...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  try {
    const { rows } = await pool.query(
      `SELECT * FROM planes_operacion WHERE id = $1`,
      [req.params.id]
    )
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO planes_operacion
         (proceso, objetivo, criterios, recursos, controles, responsable, fecha_revision, estado, creado_por)
  ...
  try {
    const { rows } = await pool.query(
      ...
      [proceso, objetivo, criterios || null, recursos || null,
       controles || null, responsable || null, fecha_revision || null, estado, id]
    )
    if (!rows[0]) return res.status(404).json({ error: 'Plan no encontrado' })
    res.json(rows[0])
  } catch (err) {
...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
  try {
    const { rowCount } = await pool.query(
      `DELETE FROM planes_operacion WHERE id=$1`, [req.params.id]
    )
